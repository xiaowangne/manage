  ...
  <div class="create-type-section">
    ...
    <div class="create-type-form">
      <el-form
        ...
        status-icon
      >
        <el-form-item label="文章类型名称" prop="title">
        ...
        <el-form-item label="页数">
          <el-input-number v-model="typeData.pageNum" :min="1" :max="100" />
        </el-form-item>
        <el-form-item>
...
const getTypeDataByTypeId = () => {
  proxy.$axios
    ...
    .then((result) => {
      ...
      } else if (result.data.status == 1080) {
        ElMessage({
          showClose: true,
          message: result.data.msg,
          type: "success",
...
const createType = (formData, config) => {
  proxy.$axios
    .post("/createType", formData, config)
    .then((result) => {
      if (result.data.status == 1041) {
        ElMessage({
          showClose: true,
        ...
        return router.push({ name: "Login" });
      } else if (result.data.status == 1050) {
        ElMessage({
          showClose: true,
          message: result.data.msg,
          type: "success",
        });
        return router.push({ name: "ArticleTypeList" });
      } else {
        ElMessage({
          showClose: true,
          message: result.data.msg,
...
const editType = ( config) => {
  ...
  values["type"] = "ArticleType";
  let editData = processoFrmData(values);
  
  proxy.$axios
    .post("/editType", editData, config)
    .then((result) => {
      if (result.data.status == 1041) {
        ElMessage({
          showClose: true,
        ...
        return router.push({ name: "Login" });
      } else if (result.data.status == 1090) {
        ElMessage({
          showClose: true,
          message: result.data.msg,
          type: "success",
